...
/// A didactic Merkle Tree for batching 1,000s of transactions.
...
impl MerkleTree {
    ...
    pub fn prove(&self, index: usize) -> Vec<(u64, bool)> {
        ...
        for level in &self.levels {
            ...
            if sibling_idx < level.len() { proof.push((level[sibling_idx], is_right)); }
            else { proof.push((level[idx], is_right)); }
            idx /= 2;
        }
        proof
    }
    ...
    pub fn verify(root: u64, leaf_hash: u64, proof: Vec<(u64, bool)>) -> bool {
        ...
        for (sibling, is_right) in proof {
            let mut hasher = DefaultHasher::new();
            if is_right { sibling.hash(&mut hasher); current.hash(&mut hasher); }
            else { current.hash(&mut hasher); sibling.hash(&mut hasher); }
