...
impl Shield {
    ...
    #[requires(self.modulus > 0)]
    #[ensures(result < self.modulus)]
    ...
    #[requires(self.modulus > 0)]
    #[ensures(result < self.modulus)]
    #[ensures(result == (if shielded >= total_key { (shielded - total_key) % self.modulus } else { (self.modulus + shielded - total_key) % self.modulus }))]
